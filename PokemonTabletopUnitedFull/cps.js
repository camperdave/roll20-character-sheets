...
on("chat:message", function (msg) {
	...
	if (command == "!power") {
		...
		var PowerCard = {};
		var modifiers = {};
		modifiers.NumberOfAttacks = 1;
		modifiers.NumberOfDmgRolls = 1;
        modifiers.NumberOfRolls = 1;
        modifiers.customRollFunction = undefined;
        modifiers.replaceRolls = true;
		var Tag = "";
		var Content = "";
		...
		while (n[a]) {
			...
			if (Tag.substring(0, 6).toLowerCase() == "attack") {
				modifiers.NumberOfAttacks = Tag.substring(6);
				if (modifiers.NumberOfAttacks === 0 || !modifiers.NumberOfAttacks) modifiers.NumberOfAttacks = 1;
				Tag = "attack";
			}
			if (Tag.substring(0, 6).toLowerCase() == "damage") {
				modifiers.NumberOfDmgRolls = Tag.substring(6);
				if (modifiers.NumberOfDmgRolls === 0 || !modifiers.NumberOfDmgRolls) modifiers.NumberOfDmgRolls = 1;
				Tag = "damage";
			}
            if (Tag.substring(0, 9).toLowerCase() == "multiroll") {
                modifiers.NumberOfRolls = Tag.substring(9);
				if (modifiers.NumberOfRolls === 0 || !modifiers.NumberOfRolls) modifiers.NumberOfRolls = 1;
				Tag = "multiroll";
            }
			PowerCard[Tag] = Content;
			a++;
		}
		PowerCardScript.showCard(PowerCard, modifiers, msg);   
	}
});


PowerCardScript.showCard = function(PowerCard, modifiers, msg) {
	...
	while (KeyCount < Keys.length) {
		...
		if (ReservedTags.indexOf(Tag) != -1) {
			...
			if (Tag.toLowerCase() == "attack") {
				for (var AttackCount = 0; AttackCount < modifiers.NumberOfAttacks; AttackCount++) {
			...
			if (Tag.toLowerCase() == "damage") {
				for (var DamageCount = 0; DamageCount < modifiers.NumberOfDmgRolls; DamageCount++) {
					RowBackground = (RowNumber % 2 == 1) ? OddRow : EvenRow;
					RowNumber += 1;
            ...
            if (Tag.toLowerCase() == "multiroll") {
                for (var MultiRoll = 0; MultiRoll < modifiers.NumberOfRolls; MultiRoll++) {
                    RowBackground = (RowNumber % 2 == 1) ? OddRow : EvenRow;
					RowNumber += 1;
	...
	var Count = 0;
	if (msg.inlinerolls !== undefined && modifiers.replaceRolls) {
